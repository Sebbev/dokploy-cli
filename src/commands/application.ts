...
import { ApplicationService, EnvironmentService, ProjectService } from "@/generated/dokploy";
import type { Application, Environment, Project } from "@/types";
...
function deploy(program: Command) {
	program
		.command("deploy")
		.description("Deploy an application from a specified project->environment")
		.option("-p, --project [PROJECT_ID]", "Project ID")
		.option("-e, --environment [ENVIRONMENT_ID]", "Environment ID")
		...
		.action(async function () {
			...
			if (!accepted) {
				accepted = await confirm({
					message: pc.blue("Do you want to deploy the application?"),
					default: false,
				});
			...
			try {
				await ApplicationService.applicationDeploy({
					applicationId: app.applicationId,
				});
				console.log(pc.green(`Application '${app.name}' deployed successfully!`));
			} catch (error) {
				if (error instanceof Error) {
					this.error(pc.red(`Failed to deploy application: ${error.message}`));
				} else {
					this.error(pc.red("An unknown error occurred while deploying the application."));
				}
			}
		});
}
...
function deleteApp(program: Command) {
	program
		...
		.action(async function () {
			...
			if (!accepted) {
				accepted = await confirm({
					message: pc.blue("Do you want to delete the application?"),
					default: false,
				});
			...
			try {
				await ApplicationService.applicationDelete({
					applicationId: app.applicationId,
				});
				console.log(pc.green(`Application '${app.name}' deleted successfully!`));
			} catch (error) {
				if (error instanceof Error) {
					this.error(pc.red(`Failed to delete application: ${error.message}`));
				} else {
					this.error(pc.red("An unknown error occurred while deleting the application."));
				}
			}
		});
}
