...
function pull(program: Command) {
  program
    .command("pull")
    .description(
      "Pull environment variables from a specified project->environment->service"
    )
...
function push(program: Command) {
  program
    .command("push")
    .description(
      "Push environment variables to a specified project->environment->service"
    )
    ...
    .action(async function (filePath) {
      ...
      try {
        ...
        if (isError(error, "ENOENT")) {
          this.error(
            pc.red("File does not exist, please provide a valid file path.")
          );
        }
      ...
      if ("applicationId" in selectedService) {
        try {
          ...
          if (error instanceof ApiError) {
            this.error(
              pc.red(
                `Failed to push environment variables: ${error.message} (${error.status})`
              )
            );
          } else {
