...
function login(authCommand) {
  authCommand.command("login").description("Authenticate with Dokploy").option("-t, --token <VALUE>", "API token").option("-u, --url <VALUE>", "server URL").action(async function() {
    const options = this.opts();
    const UrlSchema = pipe(string(), url(), transform((str) => str.replace(/\/+$/, "")));
    const TokenSchema = pipe(string(), minLength(1));
    let url2;
    if (!options.url) {
      url2 = parse(UrlSchema, await esm_default3({
        message: import_picocolors2.default.blue("Enter your Dokploy server URL (e.g. https://panel.dokploy.com):"),
        required: true,
        validate: (value) => {
      ...
      const result = safeParse(UrlSchema, options.url);
      if (!result.success) {
        this.error(import_picocolors2.default.red(result.issues[0].message));
      }
      url2 = result.output;
    }
    let token;
    if (!options.token) {
      token = parse(TokenSchema, await esm_default3({
        message: import_picocolors2.default.blue("Enter your authentication token:"),
        required: true,
        validate: (value) => {
          const result = safeParse(TokenSchema, value);
          return result.success ? true : result.issues[0].message;
        }
      }));
    } else {
      const result = safeParse(TokenSchema, options.token);
      if (!result.success) {
        this.error(import_picocolors2.default.red(result.issues[0].message));
      }
      token = result.output;
    }
    setMany({ token, url: url2 });
    initOpenAPIConfig();
    try {
      console.log(import_picocolors2.default.dim(`
Validating server...`));
      await UserService.userGet();
      console.log(import_picocolors2.default.green("Successfully authenticated with Dokploy!"));
    } catch (error) {
      if (error instanceof Error) {
        this.error(import_picocolors2.default.red(`Failed to authenticate with Dokploy: ${error.message}`));
      } else {
        this.error(import_picocolors2.default.red("Failed to authenticate with Dokploy"));
      }
    }
  });
}
async function verify(authCommand) {
  authCommand.command("verify").description("Verify authentication with Dokploy").action(async function() {
    initOpenAPIConfig();
    try {
      console.log(import_picocolors2.default.dim("Validating token with server..."));
      await UserService.userGet();
      console.log(import_picocolors2.default.green("Successfully authenticated with Dokploy!"));
    } catch (error) {
      if (error instanceof Error) {
...
function pull(program2) {
  program2.command("pull").description("Pull environment variables from a specified project->environment->service").argument("<file>", "File to save the environment variables to").action(async function(filePath) {
    ...
    try {
      await fs2.writeFile(filePath, pulledEnv || "", {
        flag: "wx"
      });
    } catch (error) {
      ...
      if (!overwrite) {
        this.error(import_picocolors3.default.red("Aborting"));
      }
      await fs2.writeFile(filePath, pulledEnv || "", {
        flag: "w"
      });
...
function push(program2) {
  program2.command("push").description("Push environment variables to a specified project->environment->service").argument("<file>", "File to read the environment variables from").action(async function(filePath) {
    initOpenAPIConfig();
    const confirmedPush = await esm_default2({
      message: "Are you sure you want to push environment variables? This will overwrite any existing variables.",
      default: false
    });
    if (!confirmedPush) {
      this.error(import_picocolors3.default.red("Aborting"));
    ...
    let envContents;
    try {
      envContents = (await fs2.readFile(filePath, { flag: "r" })).toString();
    } catch (error) {
      if (isError(error, "ENOENT")) {
        this.error(import_picocolors3.default.red("File does not exist, please provide a valid file path."));
      }
      throw error;
    }
    const projects = await ProjectService.projectAll();
    ...
    if ("applicationId" in selectedService) {
      try {
        ...
        if (error instanceof ApiError) {
          this.error(import_picocolors3.default.red(`Failed to push environment variables: ${error.message} (${error.status})`));
        } else {
          this.error(import_picocolors3.default.red("Failed to push environment variables"));
        }
      }
    } else {
      try {
...
program2.name("Dokploy CLI").description("A CLI tool for managing your Dokploy instance").version(version);
